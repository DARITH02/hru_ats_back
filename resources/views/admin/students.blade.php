...
<div id="deleteModal" class="modal-overlay">
    <div class="modal-box" style="max-width:400px">
        <div class="modal-body" style="text-align:center;padding:32px 24px 20px">
            ...
            <div style="font-family:var(--font-display);font-size:16px;font-weight:700;color:var(--text);margin-bottom:8px">Expel Student?</div>
        ...
        <div class="modal-body" style="padding:0; position:relative">
            ...
            <div style="margin-top:-10px; padding:0 24px 30px">
                <div style="background:var(--surface2); border:1px solid var(--border); border-radius:20px; box-shadow:var(--shadow-xl); padding:24px">
                    <div style="display:grid; grid-template-columns: 1.2fr 0.8fr; gap:20px; align-items:center">
                        ...
                        <div style="background:var(--surface3); border-radius:16px; padding:15px; text-align:center; border:1px solid var(--border)">
                            <div style="font-family:var(--font-mono); font-size:8px; color:var(--muted); text-transform:uppercase; margin-bottom:5px">ATTENDANCE</div>
                ...
                <div style="display:flex; gap:10px; margin-top:10px">
                    <button onclick="closeModal('profileModal')" class="btn-secondary" style="height:44px; flex:1; font-weight:700">CLOSE</button>
                    <button id="profileEditBtn" class="btn-primary" style="height:44px; flex:1; font-weight:700; gap:8px">
...
<div id="importModal" class="modal-overlay">
    <div class="modal-box" style="max-width:400px">
        ...
        <div class="modal-body" style="padding:24px">
            <div style="background:var(--accent)11;border:1px dashed var(--accent)44;border-radius:12px;padding:32px;text-align:center;">
                <svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="color:var(--accent);margin-bottom:16px">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                </svg>
                <div style="font-family:var(--font-display);font-size:14px;font-weight:700;color:var(--text);margin-bottom:6px">Upload CSV File</div>
                <div style="font-family:var(--font-mono);font-size:9px;color:var(--muted);margin-bottom:20px;letter-spacing:.05em">format: name, email, code, class_id</div>
                <input type="file" id="importFileInput" accept=".csv,.txt" style="display:none" onchange="handleFileSelected(this)">
                <button type="button" onclick="document.getElementById('importFileInput').click()" class="btn-primary" style="width:100%">SELECT FILE</button>
            </div>
            <div id="importStatus" style="display:none;margin-top:16px;font-family:var(--font-mono);font-size:10px;color:var(--accent);text-align:center;font-weight:700">PROCESSING...</div>
        </div>
    </div>
</div>
...
════════════════════════════════════════════ --}}
    ...
    <div class="page-header">
        ...
        <div style="display:flex;align-items:center;gap:10px">
            ...
            <button onclick="window.open('{{ route('admin.export.students') }}', '_blank')" class="btn-secondary" style="gap:7px; background:var(--surface3); border:1px solid var(--border)">
                <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button onclick="openModal('importModal')" class="btn-secondary" style="gap:7px;border-color:var(--border)">
                <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                IMPORT BULK
            </button>
            @endif
    ...
    <div class="panel">
        ...
        <div class="catalog-toolbar">
            <div style="display:flex;align-items:center;gap:7px">
                ...
                <span style="font-family:var(--font-mono);font-size:10px;letter-spacing:.12em;color:var(--muted2)">ENROLLMENT CATALOG</span>
            </div>
            ...
            <div class="search-wrap">
                ...
                <input id="searchInput" class="search-input" type="text"
                    placeholder="Search name, code, major…" onkeyup="filterStudents(event)"
            ...
            <select class="filter-select" id="majorFilter" onchange="filterStudents()">
                ...
                @foreach($majors as $m)
                    <option value="{{ $m->id }}" {{ request('major') == $m->id ? 'selected' : '' }}>{{ strtoupper($m->name) }}</option>
                @endforeach
            </select>
        ...
        <div id="tableView">
            <div class="table-responsive">
                ...
                @forelse($students as $student)
                    @php
                        $avatarColors = [
                            ['#4f8ef7','#6aaeff'],['#34d399','#6ef0c8'],['#a78bfa','#c4b0ff'],
                            ['#f0a732','#f5c567'],['#f25757','#f87a7a'],
                        ];
                    ...
                    <tr data-id="{{ $student->id }}"
                        ...
                        {{-- Code --}}
                        <td>
                            <span style="font-family:var(--font-mono);font-size:10px;color:var(--accent);letter-spacing:.05em">
            ...
            <div id="gridBody"
                ...
                @foreach($students as $student)
                    ...
                    <div class="instructor-card fade-up"
                        ...
                        <div style="width:52px;height:52px;border-radius:50%;background:{{ $col2[0] }}22;border:2px solid {{ $col2[0] }}44;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:700;color:{{ $col2[0] }};">
                            {{ $init2 }}
                        </div>
                        ...
                        <div style="display:flex;align-items:center;gap:14px;margin-top:4px">
                            <div>
                                <div style="font-family:var(--font-display);font-size:22px;font-weight:800;color:var(--green);line-height:1">{{ $rate2 }}%</div>
                                <div style="font-family:var(--font-mono);font-size:8px;color:var(--muted);letter-spacing:.1em">ATTENDANCE</div>
                            </div>
                        </div>

                        <div style="display:flex;gap:6px;margin-top:4px">
                            <button class="action-btn btn-view" style="width:28px;height:28px" title="View" onclick="openProfile(this.closest('.instructor-card'))">
                                <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" stroke-width="2"/></svg>
                            </button>
                            <button class="action-btn btn-edit" style="width:28px;height:28px" title="Edit" onclick="openEditModal(this.closest('.instructor-card'))">
                                <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5" stroke-width="2"/></svg>
                            </button>
                            @if(Auth::user()->isSuperAdmin())
                            <button class="action-btn btn-del" style="width:28px;height:28px" title="Remove" onclick="openDeleteModal({{ $student->id }}, '{{ addslashes($student->user->name) }}')">
                                <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M6 18L18 6M6 6l12 12" stroke-width="2.5"/></svg>
                            </button>
                            @endif
                        </div>
                    </div>
